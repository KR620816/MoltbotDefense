...
## 🛡️ Guardian Plugin (Multi-Defense Module)

Moltbot now includes the **Guardian Plugin**, a comprehensive multi-layer defense system designed to protect against sophisticated AI attacks, including prompt injection, memory poisoning, and multimodal threats.

### 5-Layer Defense Architecture

1.  **Layer 1: Input Blocking (Perimeter Defense)**
    *   **Strict Allowlisting**: Only authorized users can interact.
    *   **DM Pairing**: Mandatory pairing flow to prevent unauthorized direct messages.
    *   **Rate Limiting**: Protects against DoS and flooding attacks.

2.  **Layer 2: Input Validation (Content Filtering)**
    *   **Malicious Pattern Detection**: Hybrid Regex/AI filtering to catch known injection attacks (e.g., "Ignore previous instructions").
    *   **Multimodal Scanning**: Pre-scan images and documents for hidden instructions before processing.
    *   **Indirect Injection Defense**: Separated Reader Agent specifically for untrusted external content.

3.  **Layer 3: Execution Isolation (Sandboxing)**
    *   **Docker Sandboxing**: Sessions run in isolated containers.
    *   **Tool Denylists**: Critical tools (e.g., `exec`, `write`) are blocked for unverified inputs.
    *   **Read-Only Mode**: File system access restricted to read-only where appropriate.

4.  **Layer 4: Privilege Separation**
    *   **Role-Based Agents**: Separation of duties (Reader vs. Writer vs. Executor agents).
    *   **Credential Vault**: Secure storage of keys with minimized exposure.

5.  **Layer 5: Monitoring & Active Response**
    *   **Anomaly Detection**: Real-time monitoring of tool call patterns and resource usage.
    *   **Kill Switch**: Automated emergency shutdown upon detecting critical breaches.
    *   **P2P Pattern Sharing**: Distributed ledger system to share new attack patterns across your Moltbot nodes in real-time.

Enable the Guardian plugin in your `moltbot.json` to activate these advanced protections.
